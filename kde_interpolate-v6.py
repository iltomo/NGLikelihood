...
def log_like(dataset, wtheta_values):
    ...
    aux_kde = []
    for i in range(len(wtheta_trn)):
        aux_kde.append(KDE_pdf(dataset[i], kernel = 'gaussian'))
...
#Including the correspondent theta values
theta = np.genfromtxt("multi_galaxy_xi/theta.txt")
mg_xi['theta'] = theta

# Estimate Likelihood for each redshift bin
aux_wtheta = []
for i in range(1,6):
