...
void * setup(c_datablock * options)
{
    ...
    if (corr_type==shear_shear){
      ...
      status |= c_datablock_get_string_default(options, OPTION_SECTION, "input_section_name", "multi_galaxy_cl", &(config->input_section));

      sprintf(output_string,"multi_galaxy_%s",filter_string);     
      status |= c_datablock_get_string_default(options, OPTION_SECTION, "output_section_name", output_string, &(config->output_section));
    ...
    for (int i_bin=1; i_bin<=num_z_bin_A; i_bin++) {
        for (int j_bin=1; j_bin<=num_z_bin_B; j_bin++) {
          for (int number_of_bin = 1; number_of_bin <= 100; number_of_bin++){
            ...
            snprintf(name_out, 64, "bin_%d_%d_%d",i_bin,j_bin, number_of_bin);
